...
def main():
    ...
    save_checkpoint({
                ...
                'best_acc': best_acc,
                'optimizer' : optimizer.state_dict(),
            }, False, checkpoint=args.checkpoint,filename='best_bin.pth.tar')
    ...
    save_checkpoint({
                ...
                'best_acc': best_acc,
                'optimizer' : optimizer.state_dict(),
            }, True, checkpoint=args.checkpoint,filename='best_float.pth.tar')
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        for name, p in model.named_parameters():
            if 'weight' in name and 'bn' not in name:
                tensor = p.data    
                p.data = torch.where(tensor > 1, torch.full_like(p.data, 1), p.data)
                p.data = torch.where(tensor < -1, torch.full_like(p.data, -1), p.data)
